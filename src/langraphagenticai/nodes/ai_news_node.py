...
class AINewsNode:
    def __init__(self, llm):
        ...
        self.tavily = TavilyClient(api_key=tavily_key) if tavily_key else TavilyClient()
        self.llm = llm
    ...
    def fetch_news(self, state: dict) -> dict:
        ...
        """
        raw_msg = state['messages'][0]
        if hasattr(raw_msg, 'content'):
            frequency = str(raw_msg.content).lower()
        else:
            frequency = str(raw_msg).lower()
